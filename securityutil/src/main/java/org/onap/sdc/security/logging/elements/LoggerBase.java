...
import org.onap.sdc.security.logging.api.ILogger;
...
public abstract class LoggerBase implements ILogger {
    ...
    public static String getRequestId(HttpServletRequest httpRequest) {
        ...
        String requestId = httpRequest.getHeader(EcompHeadersConstants.X_REQUEST_ID);
        String transactionReId = httpRequest.getHeader(EcompHeadersConstants.X_TRANSACTION_ID_HEADER);
        String ecompRequestId = httpRequest.getHeader(EcompHeadersConstants.X_ECOMP_REQUEST_ID_HEADER);
    ...
    public static String getPartnerName(HttpServletRequest httpRequest) {
        String userId = httpRequest.getHeader(EcompHeadersConstants.USER_ID_HEADER);
        String onapPartnerName = httpRequest.getHeader(ONAPLogConstants.Headers.PARTNER_NAME);
        String reqUri = httpRequest.getHeader(EcompHeadersConstants.USER_AGENT_HEADER);
        return Arrays.asList(userId, onapPartnerName, reqUri).stream()
                .filter(pn-> !StringUtils.isEmpty(pn)).findFirst().orElse(EcompHeadersConstants.PartnerName_Unknown);
    }

    protected void setKeyRequestIdIfNotSetYet() {
        if (StringUtils.isEmpty(ecompLogFieldsHandler.getKeyRequestId())) {
            setKeyRequestId(generateKeyRequestId());
        }
    }

    protected void setKeyInvocationIdIfNotSetYet() {
        if (StringUtils.isEmpty(ecompLogFieldsHandler.getKeyInvocationId())) {
            setKeyInvocationId("IDAN--setKeyInvocationIdIfNotSetYet");
        }
    }
