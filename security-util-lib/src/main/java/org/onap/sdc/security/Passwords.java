...
public class Passwords {
	...
	private static Logger log = Logger.getLogger( Passwords.class.getName());
	...
	public static boolean isExpectedPassword(String password, String expectedHash) {
		...
		if (!expectedHash.contains(":")){
			log.error(EcompLoggerErrorCode.BUSINESS_PROCESS_ERROR, LogFieldsMdcHandler.getInstance().getServiceName(), new ErrorLogOptionalData(),"invalid password expecting hash at the prefix of the password (ex. e0277df331f4ff8f74752ac4a8fbe03b:6dfbad308cdf53c9ff2ee2dca811ee92f1b359586b33027580e2ff92578edbd0)\n" +
					"\t\t\t");
			return false;
		}
	...
	public static boolean isExpectedPassword(String password, String salt, String hash) {
		if ( password == null &&  hash == null )
			return true;
		if ( salt == null ){
			log.error(EcompLoggerErrorCode.BUSINESS_PROCESS_ERROR, LogFieldsMdcHandler.getInstance().getServiceName(), new ErrorLogOptionalData(),"salt must be initialized");
			return false;
		}
		//unintialized params
		if ( password == null ||  hash == null )
